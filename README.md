...
- 保持 LCD“16:9 上屏—中部硬件带—16:3 条屏”的三段式物理结构，严禁合屏
...
4. 将真实环境图与已确认带框图交给 Image 2 整体融合，保留电梯门、按钮、扶手、墙缝、地面、天花、反射和安装深度；环境主图不做后贴修补。
5. 对 LCD 三段式结构、多人尺度、多台同层 LCD 同播，以及小字和二维码做最终检查；任意 LCD 合成单屏都直接重做。
...
以下示例均为 Image 2 直接生成的虚构品牌饮品广告，只用于展示媒体框体与空间效果，不对应真实客户或投放案例。
...
环境主图保留品牌、主标题、产品主体和主要广告构图。需要生产级精确小字或可扫描二维码时，另交付标准带框图或近景图，不在环境主图上后贴修补。
